...
This document is the published code-signing policy for **FastChm**.
...
```
Get-FileHash .\FastChm-windows-x86_64.exe -Algorithm SHA256
# compare against FastChm-windows-x86_64.exe.sha256
```

Provenance (GitHub CLI):

```
gh attestation verify FastChm-windows-x86_64.exe --repo yeroo/FastChm
```
...
FastChm is a local command-line tool. It performs no network access and
collects, stores, or transmits **no user data** of any kind.
